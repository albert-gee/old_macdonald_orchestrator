        ...
        switch (event_id) {
            ...
            case OPENTHREAD_EVENT_DATASET_CHANGED:
                ...
                if (err == ESP_OK) {
                    ...
                    err = websocket_send_message_to_all_clients(response);
                    if (err != ESP_OK) {
                        ESP_LOGE(TAG, "Failed to send dataset change message to WebSocket clients");
                    }
                } else {
            ...
            case OPENTHREAD_EVENT_ROLE_CHANGED:
                ...
                if (err == ESP_OK) {
                    ESP_LOGI(TAG, "Device role changed to: %s", device_role_name);

                    response = create_json_response("thread_role_set", device_role_name);
                    err = websocket_send_message_to_all_clients(response);
                    if (err != ESP_OK) {
                        ESP_LOGE(TAG, "Failed to send device role change message to WebSocket clients");
                    }
                } else {
                    ESP_LOGE(TAG, "Failed to get device role name");
                }
            ...
            case OPENTHREAD_EVENT_START:
                ...
                response = create_json_info_response("OpenThread stack started");
                err = websocket_send_message_to_all_clients(response);
                if (err != ESP_OK) {
                    ESP_LOGE(TAG, "Failed to send OpenThread start message to WebSocket clients");
                }
                break;

            case OPENTHREAD_EVENT_STOP:
                ESP_LOGI(TAG, "OpenThread stack stopped");
                response = create_json_info_response("OpenThread stack stopped");
                err = websocket_send_message_to_all_clients(response);
                if (err != ESP_OK) {
                    ESP_LOGE(TAG, "Failed to send OpenThread stop message to WebSocket clients");
                }
                break;

            case OPENTHREAD_EVENT_DETACHED:
                ESP_LOGI(TAG, "OpenThread detached");
                response = create_json_info_response("OpenThread detached");
                err = websocket_send_message_to_all_clients(response);
                if (err != ESP_OK) {
                    ESP_LOGE(TAG, "Failed to send OpenThread detached message to WebSocket clients");
                }
                break;

            case OPENTHREAD_EVENT_ATTACHED:
                ESP_LOGI(TAG, "OpenThread attached");
                response = create_json_info_response("OpenThread attached");
                err = websocket_send_message_to_all_clients(response);
                if (err != ESP_OK) {
                    ESP_LOGE(TAG, "Failed to send OpenThread attached message to WebSocket clients");
                }
                break;
            ...
            case OPENTHREAD_EVENT_IF_UP:
                ...
                response = create_json_info_response("OpenThread network interface up");
                err = websocket_send_message_to_all_clients(response);
                if (err != ESP_OK) {
                    ESP_LOGE(TAG, "Failed to send OpenThread network interface up message to WebSocket clients");
                }
                break;

            case OPENTHREAD_EVENT_IF_DOWN:
                ESP_LOGI(TAG, "OpenThread network interface down");
                response = create_json_info_response("OpenThread network interface down");
                err = websocket_send_message_to_all_clients(response);
                if (err != ESP_OK) {
                    ESP_LOGE(TAG, "Failed to send OpenThread network interface down message to WebSocket clients");
                }
                break;

            case OPENTHREAD_EVENT_GOT_IP6:
                ESP_LOGI(TAG, "OpenThread stack added IPv6 address");
                response = create_json_info_response("OpenThread stack added IPv6 address");
                err = websocket_send_message_to_all_clients(response);
                if (err != ESP_OK) {
                    ESP_LOGE(TAG, "Failed to send OpenThread added IPv6 address message to WebSocket clients");
                }
                break;

            case OPENTHREAD_EVENT_LOST_IP6:
                ESP_LOGI(TAG, "OpenThread stack removed IPv6 address");
                response = create_json_info_response("OpenThread stack removed IPv6 address");
                err = websocket_send_message_to_all_clients(response);
                if (err != ESP_OK) {
                    ESP_LOGE(TAG, "Failed to send OpenThread removed IPv6 address message to WebSocket clients");
                }
                break;

            case OPENTHREAD_EVENT_MULTICAST_GROUP_JOIN:
                ESP_LOGI(TAG, "OpenThread stack joined IPv6 multicast group");
                response = create_json_info_response("OpenThread stack joined IPv6 multicast group");
                err = websocket_send_message_to_all_clients(response);
                if (err != ESP_OK) {
                    ESP_LOGE(TAG, "Failed to send OpenThread joined IPv6 multicast group message to WebSocket clients");
                }
                break;

            case OPENTHREAD_EVENT_MULTICAST_GROUP_LEAVE:
                ESP_LOGI(TAG, "OpenThread stack left IPv6 multicast group");
                response = create_json_info_response("OpenThread stack left IPv6 multicast group");
                err = websocket_send_message_to_all_clients(response);
                if (err != ESP_OK) {
                    ESP_LOGE(TAG, "Failed to send OpenThread left IPv6 multicast group message to WebSocket clients");
                }
                break;

            case OPENTHREAD_EVENT_TREL_ADD_IP6:
                ESP_LOGI(TAG, "OpenThread stack added TREL IPv6 address");
                response = create_json_info_response("OpenThread stack added TREL IPv6 address");
                err = websocket_send_message_to_all_clients(response);
                if (err != ESP_OK) {
                    ESP_LOGE(TAG, "Failed to send OpenThread added TREL IPv6 address message to WebSocket clients");
                }
                break;

            case OPENTHREAD_EVENT_TREL_REMOVE_IP6:
                ESP_LOGI(TAG, "OpenThread stack removed TREL IPv6 address");
                response = create_json_info_response("OpenThread stack removed TREL IPv6 address");
                err = websocket_send_message_to_all_clients(response);
                if (err != ESP_OK) {
                    ESP_LOGE(TAG, "Failed to send OpenThread removed TREL IPv6 address message to WebSocket clients");
                }
                break;

            case OPENTHREAD_EVENT_TREL_MULTICAST_GROUP_JOIN:
                ESP_LOGI(TAG, "OpenThread stack joined TREL IPv6 multicast group");
                response = create_json_info_response("OpenThread stack joined TREL IPv6 multicast group");
                err = websocket_send_message_to_all_clients(response);
                if (err != ESP_OK) {
                    ESP_LOGE(
                        TAG, "Failed to send OpenThread joined TREL IPv6 multicast group message to WebSocket clients");
                }
                break;

            case OPENTHREAD_EVENT_SET_DNS_SERVER:
                ESP_LOGI(TAG, "OpenThread stack set DNS server");
                response = create_json_info_response("OpenThread stack set DNS server");
                err = websocket_send_message_to_all_clients(response);
                if (err != ESP_OK) {
                    ESP_LOGE(TAG, "Failed to send OpenThread set DNS server message to WebSocket clients");
                }
                break;

            case OPENTHREAD_EVENT_PUBLISH_MESHCOP_E:
                ESP_LOGI(TAG, "OpenThread stack started publishing meshcop-e service");
                response = create_json_info_response("OpenThread stack started publishing meshcop-e service");
                err = websocket_send_message_to_all_clients(response);
                if (err != ESP_OK) {
                    ESP_LOGE(TAG, "Failed to send OpenThread publishing meshcop-e service message to WebSocket clients")
                    ;
                }
                break;
            ...
            case OPENTHREAD_EVENT_REMOVE_MESHCOP_E:
                ...
                response = create_json_info_response("OpenThread stack removed meshcop-e service");
                err = websocket_send_message_to_all_clients(response);
                if (err != ESP_OK) {
                    ESP_LOGE(TAG, "Failed to send OpenThread removed meshcop-e service message to WebSocket clients");
                }
                break;
